...
if uploaded_file:
    ...
    if core_probs:
        ...
        if score >= 50:
            ...
            * **學力斷層建議：** 核心科目高分者過少，建議強化教學機制，增加對基礎學科的輔導，並針對頂尖學生提供更多進階訓練。
            """)

        # 展開查看分析了哪些課
        with st.expander("🔍 查看 AI 分析的課程細節"):
            st.write(f"本次 AI 鑑定共參考了 **{len(found_courses)}** 門核心課程，包含：")
            st.write(", ".join(set(found_courses)))

    else:
        st.error("❌ 鑑定失敗！CSV 中找不到符合關鍵字的課程（微積分、線性代數、代數、分析等）。請檢查檔案格式。")

# --- 5. 側邊欄資訊 ---
with st.sidebar:
    st.header("關於此系統")
    st.info("本系統使用 1D-CNN 卷積模型分析大學部成績分布 (Grading Distribution)。")
    st.write("訓練資料集包含多所大學數學系之歷年公開數據。")
    st.caption("AI 預測僅供學術參考之用。")